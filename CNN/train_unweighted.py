#!/usr/bin/env python3
"""
Train CNN WITHOUT class weights to compare performance on imbalanced dataset.
This version uses standard (unweighted) CrossEntropyLoss.
"""
import argparse
import random
import sys
...
def main():
    parser = argparse.ArgumentParser(description="Train CNN WITHOUT class weights (unweighted loss).")
    ...
    # UNWEIGHTED cross entropy - no class weights applied
    print("=" * 80)
    print("USING UNWEIGHTED LOSS (no class weights)")
    print("This will likely bias towards majority classes on imbalanced data")
    print("=" * 80)
    ...
    if ckpt is not None:
        try:
            ...
            # Force learning rate to args.lr
